...
export function VerticalMenu({type, title, atr, setPrismPath, setPrismData, opsShowing, setOpsShowing}) {
  ...
  useEffect(() => {
    let url = 'noop';
    (type !== "") ? url = `http://localhost:1200/rest/api/ui/vertical-menu/${type}` : '';
  ...
  return (
    <StyledVerticalMenu atr={atr}>
      ...
      {elements != null && elements.map((el) => {
        console.log(el)
        return <div className={el.type}>
          <DynamicIcon className={`${el.type}-icon`} svgIconUrl={`/icons/20px/${el.icon}.svg`}/>
          <div className={`${el.type}-text`} onClick={(e) => doAction(el.action)}>{el.text}</div>
        </div>
      }
      )}
      {dfs !== null && dfs.length === 1 &&
      <FileSystem key={dfs.component_type} path={dfs[0]['path']} fsType={"Documentation"} setPrismPath={setPrismPath} setPrismData={setPrismData}/>
      }
      {bfs !== null && bfs.length === 1 &&
      <FileSystem key={bfs.component_type} path={bfs[0]['path']} fsType={"Browser"} setPrismPath={setPrismPath} setPrismData={setPrismData}/>
      }
