	...
	& #modal-center-dialog-container {
		...
		& #modal-center-dialog-content {
			...
			& #modal-submit-login {
				...
				border-radius: 2px;
				font-family: 'Hackman-Bold',serif;
			}
...
export const Modal = ({modalShowing, setModalShowing, modalDisplay, validjwt, validcreds, request, userAvail, setUserAvail}) => {
  ...
  const doRequest = async () => {
    switch (modalDisplay) {
      ...
      case "signup":
        setTrySignup(true);
        if (emailValid === false || fullName === "" || !userAvail || email === "") {
          break;
        }
        if (isLong === true && hasNumber === true && hasSpecial === true && hasUpper === true && hasLower === true) {
          setPasswordValid(true);
        } else {
          setPasswordValid(false);
          break;
        }
        request('noop', 'create-user', JSON.stringify(
          {user: email, fullname: fullName, password: password}));
        break;
      default:
        break;
    }
  ...
  const waitTestEmail = (value) => {
    ...
    valid_email_timeout = setTimeout(() => {
      let ev = emailIsValid(value);
      (value === "") ? ev = true : '';
      if (value !== "" && ev === true) {
        request("noop", "rapid-test-user-avail", value)
        setEmailValid(ev);
      }
    }, 500);
  }

  let valid_password_timeout = null;
  ...
  return (
    <StyledModal onClick={(e) => setModalShowing(false)}
                 showing={modalShowing} emailValid={emailValid}
                 modalDisplay={modalDisplay} pwdlong={isLong}
                 pwdisupper={hasUpper} pwdislower={hasLower}
                 pwdisspecial={hasSpecial} pwdisnumeric={hasNumber}>
      <div id="modal-center-dialog-container"
           onClick={(e) => e.stopPropagation()}>
        <div id="modal-center-dialog-header">
          <div id="modal-center-dialog-header-text">{headerText}</div>
          <div id="close-modal"
               onClick={(e) => setModalShowing(false)}>{}</div>
        </div>
        <div id="modal-center-dialog-content">
          {(validcreds !== null && validcreds === false) &&
          <Alerts type="danger" msg="Invalid Credentials. Please make sure your username and password are correct." showIcon={false}/>}

          {(modalDisplay === 'signup' && passwordValid === false && trySignup === true) &&
          <Alerts type="danger" msg="Password Validation Errors." ulErrorLogic={[hasUpper, hasLower, hasNumber, isLong, hasSpecial]} ul={validationMessages} showIcon={false}/>}

          {((modalDisplay === 'signup' && trySignup === true) && (fullName === "" || email === "")) &&
          <Alerts type="danger" msg="All Fields Requied." showIcon={false}/>}

          {(modalDisplay === 'signup' && !userAvail) &&
          <Alerts type="danger" msg="Email Account Exists..." showIcon={false}/>}
          ...
          <div id="modal-submit-login"
               onClick={(e) => doRequest()}>{submitText}</div>
        </div>
      </div>
    </StyledModal>
  )
