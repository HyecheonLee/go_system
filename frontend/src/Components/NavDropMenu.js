...
export function NavDropMenu({navDropMenuPosX, setNavDropMenuPosX, navDropMenuType,doLogOut}) {
  ...
  return (
    <StyledNavDropMenu ndmpx={navDropMenuPosX} navDropMenuType={navDropMenuType}>
      {jsonData != null && jsonData.elements.map((el, index) =>

        <div key={`${jsonData._id}_${index}`} className={el.type} onClick={(e) => doAction(el.action)}>
          <DynamicIc svgIconUrl={`/icons/20px/${el.icon}.svg`}/>{el.text}
        </div>
      )}
    </StyledNavDropMenu>
  )
